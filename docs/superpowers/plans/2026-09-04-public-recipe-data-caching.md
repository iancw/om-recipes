...
**Known limitation (by design, not a gap) — resolved:** this plan did not eliminate `getAuthedAuthorIds()` on `app/recipes/[id]/page.jsx` — a live, session-scoped `authors` lookup that ran for every *logged-in* viewer to compute `isOwner`. That per-viewer lookup has since been folded into the per-user Blobs cache from §§1-8 of the design spec (`lib/user-state-cache.js`'s `getUserSavedState` now hydrates `authorIds` alongside `savedRecipeIds`, and `addAuthorIdToUserState` keeps it current when a viewer's first author row is created), so logged-in recipe-page views no longer run this query either once that viewer's blob is hydrated.
