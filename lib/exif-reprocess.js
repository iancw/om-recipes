...
/**
 * A sample image is only a usable EXIF source when it has a prepared object
 * key to download AND is either finalized OR has published renditions
 * (`smallUrl` set). The `smallUrl` clause admits fully-migrated legacy
 * images that import scripts never stamped `finalizedAt` on. Exported so the
 * orchestrator can apply the same predicate when deciding which images to
 * fetch — seeding a fetch for an unusable row produces spurious
 * `download_failed` noise.
 */
export function isUsableSampleImage(sample) {
    return Boolean(sample?.preparedObjectKey)
