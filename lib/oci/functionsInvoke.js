...
function requireImageResizeFunctionId() {
    const v = process.env.OCI_PROCESS_IMAGE_FUNCTION_ID || process.env.OCI_IMAGE_RESIZE_FUNCTION_ID;
    if (!v) throw new Error('Missing OCI_PROCESS_IMAGE_FUNCTION_ID or OCI_IMAGE_RESIZE_FUNCTION_ID env var');
    return v;
}
...
export async function invokeImageResizeFunction({ sourceBucket, objectName, objectNames, destinationBucket, timeoutMs }) {
    const functionId = requireImageResizeFunctionId();
