...
// A request stuck in "processing" past this age almost certainly means the
// function that was building it got killed (e.g. platform execution timeout)
// before it could record success or failure. Without this, a single killed
// run would block that user from ever retrying.
const STALE_PROCESSING_REQUEST_MS = 5 * 60 * 1000;

const ORIGINAL_UPLOAD_DOWNLOAD_CONCURRENCY = 6;
...
async function getInFlightPrivacyRequest(userId, requestType) {
    ...
    if (existing.status === PRIVACY_REQUEST_STATUS_PROCESSING) {
        const updatedAt = existing.updatedAt instanceof Date ? existing.updatedAt : new Date(existing.updatedAt);
        if (Date.now() - updatedAt.getTime() > STALE_PROCESSING_REQUEST_MS) {
            await updatePrivacyRequest(existing.id, {
                status: PRIVACY_REQUEST_STATUS_FAILED,
                failureSummary: 'The request stopped making progress and was marked as failed. Please try again.',
...
async function collectUserExportPayload(userId) {
    ...
    const [sampleRows, comparisonRows, savedRows, modeAssignmentRows, notificationRows, notificationPreferenceRows] = await Promise.all([
        imageIds.length === 0
            ? []
            : db
                  .select({
...
async function fetchOriginalUploadEntry({ client, namespaceName, image }) {
    const objectKey = String(image.preparedObjectKey ?? '').trim();
    if (!objectKey) {
        return {
            missing: { imageId: image.id, reason: 'No original object key is recorded for this upload' }
        };
    }
    ...
    try {
        ...
        return { entry: { name: `uploads/${image.uuid}${ext}`, data: buffer } };
    } catch (error) {
        return { missing: { imageId: image.id, objectKey, reason: String(error?.message ?? error) } };
    }
}
...
async function collectOriginalUploadEntries(imageRows) {
    ...
    const results = await mapWithConcurrency(imageRows, ORIGINAL_UPLOAD_DOWNLOAD_CONCURRENCY, (image) =>
        fetchOriginalUploadEntry({ client, namespaceName, image })
    );
    ...
    for (const result of results) {
        ...
        if (result.missing) missingOriginalUploads.push(result.missing);
    }
...
async function eraseAccountData(userId, userUuid) {
    ...
    try {
        ...
        console.error('[privacy] failed to delete user-state blob during account erasure', { userUuid, error });
    }
}
...
export async function getDownloadablePrivacyExport({ requestUuid, userId, now = new Date() }) {
    const rows = await db
        .select({
            id: privacyRequests.id,
            requestType: privacyRequests.requestType,
